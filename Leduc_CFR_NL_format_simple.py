...
class Node:
	...
	def get_strategy(self):
		...
		for a in self.bet_options:
			if normalizing_sum > 0:
				...
				self.strategy[a] = round(1.0/self.num_actions,5)
	...
	def get_average_strategy(self):
		...
		for a in self.bet_options:
			if normalizing_sum > 0:
				avg_strategy[a] = round(self.strategy_sum[a] / normalizing_sum,5)
			else:
				avg_strategy[a] = round(1.0 / self.num_actions,5)
	...
	def cfr_iterations_external(self):
		...
		with open('leducnlstrat.txt', 'w+') as f:
			for i in sorted(self.nodes):
				temp = self.nodes[i].get_average_strategy()
				temp1 = str(temp)
				temp2 ="defaultdict(<class 'int'>"
				temp3 = temp1.lstrip(temp2)
				f.write('{}, {}\n'.format(i, temp3))

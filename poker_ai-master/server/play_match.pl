...
if( $dealerPID == 0 ) {
    ...
    open STDERR, ">>$cwd/logs/$ARGV[ 0 ].err" or die "can't open log file $cwd/logs/$ARGV[ 0 ].err";
...
for( $p = 0; $p < $numPlayers; ++$p ) {
	...
	open STDOUT, ">$cwd/logs/$ARGV[ 0 ].player$p.std"
	    or die "can't dup player $p STDOUT";
	open STDERR, ">$cwd/logs/$ARGV[ 0 ].player$p.err"
